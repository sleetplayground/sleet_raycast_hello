...
export function getNetworkConfig(): NetworkPreferences {
  ...
  const nodeUrl = preferences.nodeUrl || 
    (networkId === 'mainnet' ? 'https://rpc.mainnet.near.org' : 'https://rpc.testnet.near.org');
  const walletUrl = preferences.walletUrl || 
    (networkId === 'mainnet' ? 'https://wallet.near.org' : 'https://wallet.testnet.near.org');
  const helperUrl = preferences.helperUrl || 
    (networkId === 'mainnet' ? 'https://helper.mainnet.near.org' : 'https://helper.testnet.near.org');
  const explorerUrl = preferences.explorerUrl || 
...
export function getContractName(): string {
  ...
  return preferences.contractName || 
    (networkId === 'mainnet' ? 'hello.sleet.near' : 'hello.sleet.testnet');
}
...
export async function getAccountDetails(accountId: string, networkId: string) {
  try {
    const provider = new providers.JsonRpcProvider({ url: networkId === 'mainnet' ? 
